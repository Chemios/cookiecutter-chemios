...
def test_bake_with_defaults(cookies):
    with bake_in_temp_dir(cookies) as result:
        ...
        assert 'chemios_instrument' in found_toplevel_files
...
def test_bake_withspecialchars_and_run_tests(cookies):
    """Ensure that a `developer_name` with double quotes does not break setup.py"""
    with bake_in_temp_dir(cookies, extra_context={'developer_name': 'name "quote" name'}) as result:
        assert result.project.isdir()
        run_inside_dir('python setup.py test', str(result.project)) == 0


def test_bake_with_apostrophe_and_run_tests(cookies):
    """Ensure that a `developer_name` with apostrophes does not break setup.py"""
    with bake_in_temp_dir(cookies, extra_context={'developer_name': "O'connor"}) as result:
        assert result.project.isdir()
        run_inside_dir('python setup.py test', str(result.project)) == 0
...
def test_using_pytest(cookies):
    with bake_in_temp_dir(cookies, extra_context={'use_pytest': 'y'}) as result:
        assert result.project.isdir()
        test_file_path = result.project.join('tests/test_chemios_instrument.py')
